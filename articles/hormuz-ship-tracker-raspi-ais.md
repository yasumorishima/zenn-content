...
4日間の連続収集で43,000件以上の位置データが蓄積されました。ここから新たに分かったことを報告します。

### 船舶密度ヒートマップ

![船舶密度ヒートマップ — ペルシャ湾全体とホルムズ海峡拡大](https://raw.githubusercontent.com/yasumorishima/hormuz-ship-tracker/master/docs/heatmap.png)
*左: ペルシャ湾全域の密度。Dubai付近に集中。右: 海峡拡大。赤い点線の「AIS DEAD ZONE」が受信不能エリア。下段: 港域別船舶数・船籍国・船種の分布。*
...
*24時間の船舶移動をアニメーション化。各船の位置は前後のデータポイントから線形補間し、陸地を横断しないようフィルタリング済み。*

### 通過船舶レポート

![通過船舶レポート](https://raw.githubusercontent.com/yasumorishima/hormuz-ship-tracker/master/docs/transit_report.png)

Dubai / Jebel Ali Approachゲートでは61回の通過を検知しました。一方、ホルムズ海峡ゲートの通過は0件です。

### AIS異常値の正体

データの約17%に異常値が含まれていました。分析の結果、主に2種類のパターンが特定できました。

| 異常の種類 | 件数 | 原因 |
|---|---|---|
| 速度 102.3ノット | 約3,200件 | AISプロトコルの「速度データなし」を示すセンチネル値（10bit 0x3FF） |
| 速度 40〜99ノット | 約4,100件 | 沿岸AIS受信局のデコードエラー。複数の船が同時に同じ座標・同じ速度で検知される |

特に48ノット付近の異常値は、2026-03-16 07:00 UTCに4隻が同時にホルムズ海峡付近の同一座標に出現するという特徴的なパターンでした。これは受信局1局の誤動作で、船舶が実際にそこにいたわけではありません。

この異常データが初期のtransit検知で41件の偽陽性を生んでいましたが、速度40ノット以上のデータを除外するフィルタを導入して解決しています。

ダッシュボード上では、異常値のある船舶は赤い点線マーカーで表示され、ポップアップに「DATA QUALITY WARNING」が表示されるようにしました。

### ブラウザで再生できるリプレイ機能

`/replay` エンドポイントで、Leaflet.jsベースのアニメーションリプレイが利用可能です。再生・一時停止・速度変更（0.25x〜16x）・タイムラインスクラブに対応し、キーボードショートカットも使えます（Space: 再生、矢印: コマ送り、+/-: 速度変更）。

## 制約と注意点

- **陸上AISの限界**: 無料のaisstream.ioは陸上受信局ベース。海峡中央部や沖合のカバレッジは限られる
- **AIS速度102.3ノット**: AIS仕様で「速度利用不可」を示すセンチネル値（0x3FF）。異常値ではなくフィルタが必要
- **速度40〜99ノットの受信局グリッチ**: 沿岸受信局のデコードエラーにより、複数船舶が同一座標に出現する偽データが発生する。transit検知では40ノット以上を除外
- **2分間隔のスロットリング**: 同一船舶のデータを2分間隔に間引いているため、高速通過の検出精度に影響する可能性がある
- **データ収集期間**: 継続収集中。長期的なトレンド分析には更なる蓄積が必要

## まとめ

aisstream.ioの無料APIとRaspberry Pi 5を使って、ペルシャ湾全域の船舶をリアルタイムに収集・分析するシステムを構築しました。4日間で43,000件以上のデータを収集し、ヒートマップ・タイムラプス・データ品質分析まで実装しています。

データは蓄積を続けており、統計レポートは6時間ごとに自動更新されています。
